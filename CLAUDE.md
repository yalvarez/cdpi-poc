...
- [x] **Studio OOB JSON-LD credential issuance validated**: April 19, 2026. Required five patches total: (1) utility S3→MinIO, (2) api-gateway TLD validator, (3) Credo CredentialEvents crash guard, (4) issuance schema URL dedup, (5) issuance @context triple-prefix normalization. All automated in `init-credebl.sh`. Credo patch only restarts Credo when actually applied (conditional restart avoids unnecessary JWT invalidation on re-runs).
