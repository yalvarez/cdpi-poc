...
function normalizeTheme(theme?: string | null): string | null {
  ...
  if (!value || ['default', 'undefined', 'null'].includes(value)) {
    return null
  }
...
export function ActiveThemeProvider({
  ...
  const envTheme = normalizeTheme(process.env.NEXT_PUBLIC_ACTIVE_THEME)

  /** Determine initial theme */
  const getInitialTheme = (): string => {
    const initialThemeValue = normalizeTheme(initialTheme)
    if (initialThemeValue) {
      return initialThemeValue
    }

    const cookieTheme = normalizeTheme(getThemeFromCookie())
    if (cookieTheme) {
      return cookieTheme
    }
